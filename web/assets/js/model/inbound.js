...
class xHTTPStreamSettings extends XrayCommonClass {
    constructor(
        ...
        this.uiXmuxSettings = uiXmuxSettings;
    }
    ...
    genTrojanLink(address = '', port = this.port, forceTls, remark = '', clientPassword) {
        ...
        switch (type) {
            ...
            case "xhttp":
                ...
                let trojanClient = this.settings.trojans?.find(c => c.password === clientPassword);
                if (trojanClient && trojanClient.xmuxOverride && trojanClient.xmuxOverride.enabled) {
                    for (let k in trojanClient.xmuxOverride) {
                        if (k !== 'enabled' && trojanClient.xmuxOverride[k] !== '' && trojanClient.xmuxOverride[k] !== null && trojanClient.xmuxOverride[k] !== undefined) {
                            finalXmuxObj[k] = trojanClient.xmuxOverride[k];
                        }
                    }
                }
                let isXmuxEmpty = true;
        ...
        if (json.extra) {
            try {
                ...
                if (extraObj && extraObj.xmuxOverride) {
                    client.xmuxOverride = extraObj.xmuxOverride;
                }
            } catch (e) {
...
Inbound.TrojanSettings.Trojan = class extends XrayCommonClass {
    ...
    static fromJson(json = {}) {
        let client = new Inbound.TrojanSettings.Trojan(
            json.password,
            json.email,
            json.limitIp,
            json.totalGB,
